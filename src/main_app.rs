...
impl ItemInfo {
    fn new(id: i32, name: String, price: i32, age_limit: i32) -> Self {
        ItemInfo { id, name, price, age_limit }
    }
}
            ...
            *status_check_timer = 0;
            ...
            let id = match msg.chars().skip(4).collect::<String>().parse::<i32>() {
                ...
                Err(e) => {
                    ...
                    bail!("client {} sent invalid item id: {}", addr, e);
                }
            };

            let result = task::block_in_place(|| { // 2f. check username against db
                let mut stmt = db.prepare("SELECT id, name, price_per_unit, age_limit FROM Items WHERE id = ?").unwrap();
